...
def register_admin(full_name: str, password: str, level: str) -> Tuple[bool, str, Optional[int]]:
    """Insert a new admin and return (success, message, id or None)."""
    if not (full_name and password and level):
        return False, "All fields are required.", None
    ...
    try:
        with get_conn() as conn:
            cursor = conn.execute(
                "INSERT INTO admins (full_name, password, level) VALUES (?, ?, ?)",
                (full_name, hashed, level),
            )
            admin_id = cursor.lastrowid
        return (
            True,
            f"Admin registration successful! Your ID is {str(admin_id).zfill(5)}",
            admin_id,
        )
    except Exception as exc:
        return False, f"Admin registration failed: {exc}", None


def authenticate_admin(admin_id: str, password: str) -> Optional[dict]:
    """Return a dict with admin data if credentials are correct, else None."""
    try:
        aid_normalized = _normalize_id(admin_id)
    except ValueError:
        return None
