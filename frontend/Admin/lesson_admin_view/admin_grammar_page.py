...
def load_grammar_lessons():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT content FROM grammarLesson ORDER BY id")
        lessons = [row[0] for row in cursor.fetchall()]
        conn.close()
        return lessons
    except Exception as e:
        print("Failed to load grammar lessons:", e)
        return []

def admin_grammar_page(page: ft.Page):
    ...
    top_nav = ft.Container(
        content=ft.Row(
            controls=[
                ft.Text("Grammar Courses", size=24, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
