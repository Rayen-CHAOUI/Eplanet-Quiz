...
def load_speaking_lessons():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT content FROM speakingLesson ORDER BY id")
        lessons = [row[0] for row in cursor.fetchall()]
        conn.close()
        return lessons
    except Exception as e:
        print("Failed to load speaking lessons:", e)
        return []

def admin_speaking_page(page: ft.Page):
    lessons = load_speaking_lessons()
    if not lessons:
        lessons = ["No speaking lessons found."]
    ...
    top_nav = ft.Container(
        content=ft.Row(
            controls=[
                ft.Text("Speaking Courses", size=24, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
